...
namespace TradingService.Functions.BlockManagement
{
    public class CreateBlocksFromLadder
    {
        ...
        public CreateBlocksFromLadder(IConfiguration configuration, ITradeService tradeService, IBlockItemRepository blockRepo, ILadderItemRepository ladderRepo, IAccountItemRepository accountRepo)
        {
            ...
            ILogger log)
        {
            ...
            try
            {
                currentPrice = await _tradeService.GetCurrentPrice(_configuration, userId, ladderData.Symbol);
            }
            catch (Exception ex)
            {
            ...
            var blockPrices = GenerateBlockPricesByPrice(accountType, currentPrice, ladderData.BuyPercentage, ladderData.SellPercentage, ladderData.StopLossPercentage).OrderBy(p => p.BuyPrice);
